...
def index(request):
    ...
    str_sql = """
        exec NOVA..sp_compras_planeacion_demanda;
    """
    ...
    for row in obj_data:
        if row["NoProducto"] not in arr_planeacion_sku:
            arr_planeacion_sku[row["NoProducto"]] = {
                ...
                "Nivel2": row['Nivel2'],
                "Nivel3": row['Nivel3'],
                # "Libras_Mes_3": row['Libras_Mes_3'],
                "Libras_Mes_3": 0,
                # "Libras_Mes_2": row['Libras_Mes_2'],
                "Libras_Mes_2": 0,
        ...
        arr_planeacion_sku[row["NoProducto"]]["Libras_Mes_3"] += row['Libras_Mes_3']
        arr_planeacion_sku[row["NoProducto"]]["Libras_Mes_2"] += row['Libras_Mes_2']
        ...
        arr_planeacion_tabla[row["NoProducto"]]["Libras_Mes_2"] += row['Libras_Mes_2']
    ...
    month_2 = (13 if month_1 == 1 else month_1) - 1
    month_3 = (13 if month_2 == 1 else month_2) - 1
    ...
    for k in arr_planeacion_tabla:
        ...
        arr_info_excel.append({
            ...
            "Libras_Mes_2": format(producto['Libras_Mes_2']),
    ...
    data = {
        ...
        "month_2": MESES[month_2],
        "month_3": MESES[month_3],
    }
    
    bool_json = request.POST.get('bool_json', '')
    if not bool_json or bool_json == '' or bool_json == False:
        return render(request, 'planeacion_demanda/planeacion_demanda.html', data)
    else:
        data['status'] = True
...
def get_inventario_transito_pedido(request):
    ...
    str_sql = """ DECLARE @nivel3 VARCHAR(100) = '%s', @fecha DATE = '%s'
        EXEC NOVA..sp_planeacion_demanda_%s @nivel3, @fecha
    """ % (str_nivel3, str_fecha, str_tipo)
