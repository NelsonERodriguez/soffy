...
async function drawInfoTableMonths(objResult, intPeriod) {
    ...
    for(let k in objResult){
        const d = objResult[k];
        let intLB = (d.utilizacion * 1),
            intTotal = (d.librascapacidad * 1),
            intCosto = (d.porcentaje_mes * 100);
        intLB = intLB.toFixed(0);
        intCosto = intCosto.toFixed(0);
        ...
        tdRowPresupuesto += `<td class='tdDetailNoMargin' >${intTotal} lb</td>`;
        tdCosto += `<td class='tdDetailNoMargin' >${intCosto} %</td>`;
        let pb = ( d.periodo_base * 100);
        let objetivo = (d.objetivo * 100);
        arrReturn[k] = {
            'month': `Mes ${d.mes}`,
            'real': (intCosto * 1),
            'period': (objetivo * 1),
...
function drawChartMonths(objData) {
    ...
    const objGraphic = {
        'str_id': 'chartMonths',
        'title': 'Indicador Utilizacion de Bodega de los Ultimos Cuatro Meses (según filtro)',
        'subTitle': '',
...
function drawChartWeeks(objData) {
    ...
    const objGraphic = {
        'str_id': 'chartWeeks',
        'title': 'Indicador Utilizacion de Bodega de las Ultimas Cuatro Semanas (según filtro)',
        'subTitle': '',
        'strObjective': strObjective,
        'strAlcance': strAlcance,
        'objCategories': objCategories,
