(function () {
    ...
    fileXML.onchange = (e) => {
        ...
        inputOrden.setAttribute('disabled', 'disabled');
        resetFacturaXML();
        showBtnSave(false);
                        ...
                        if (e.rowType === "data") {
                            if (objFacturaOrden.detalles) {
                                ...
                                for (let i = 0; i < objFacturaOrden.detalles.length; i++) {
                                    ...
                                    if (parseFloat(detalle.total) === parseFloat(e.data.total)) {
                                        encontrado = true;
                                        break;
                                    }
                                }
        ...
        fetch(urlSaveContrasenaPublic, {
            ...
            .then((data) => {
                ...
                if (data.status) {
                    ...
                    window.open(urlPdfContrasenaPublic.replace('/0/', `/${data.contrasena}/`).replace('/1/', `/${data.empresa}/`));
                    showBtnSave(false);
                    setTimeout(() => {
